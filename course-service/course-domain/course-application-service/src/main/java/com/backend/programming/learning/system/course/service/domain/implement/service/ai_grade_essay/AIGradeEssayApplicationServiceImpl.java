...
public class AIGradeEssayApplicationServiceImpl implements AIGradeEssayApplicationService {
    ...
    public String gradeEssay(AssignmentAIGradeReport assignmentAIGradeReport) throws JsonProcessingException {
        String apiUrl = String.format(API_URL_TEMPLATE, "AIzaSyCzF7o3Z3RTO8Bqo6s18Yj18WyFeqZrvVM");
...
I. SYSTEM_INSTRUCTIONS:
    ...
    C. Example Process for a Single Submission:
        ...
        Rubric rubric = objectMapper.readValue(rubricUser.getContent(), Rubric.class);
        String questionRubric  = formatRubric(rubric);
        String INPUT_OUTPUT = interpolateInputOutput(studentSubmissions, totalSubmissions, questionContent, questionMaxScore, questionRubric, language);
        ...
        assignmentAIGradeReport.setFeedbackSubmissions(text.replaceAll("```", "").replaceAll("json", ""));
        ...
        return text.replaceAll("```", "").replaceAll("json", "");
    }

    @Override
    public void createReportEssay(UUID assignmentId, UUID rubricId) throws JsonProcessingException {
        ObjectMapper objectMapper = new ObjectMapper();
        ...
        Optional<RubricUser> rubricUserOptional = rubricUserRepository.findRubricUser(rubricId);
        if (rubricUserOptional.isEmpty()) {
            throw new RuntimeException("RubricUser not found");
        }
        ...
        assignmentAIGradeReport.setRubricUser(rubricUserOptional.get());
    ...
    public String interpolateInputOutput(String studentSubmissions, long studentSubmissionsLength, String questionContent, Float questionMaxScore, String questionRubrics, String language) {
        String template = """
                II. INPUT AND OUTPUT:
                B. Input:
        ...
        valuesMap.put("questionRubrics", questionRubrics);
    ...
    public String formatRubric(Rubric rubric) {
        StringBuilder formattedData = new StringBuilder();

        rubric.getCriteria().forEach(criteria -> {
            formattedData.append("- Criteria: ").append(criteria.getCriteriaName())
                    .append(" (Total score: ").append(criteria.getCriteriaGrade()).append("%)\n");
