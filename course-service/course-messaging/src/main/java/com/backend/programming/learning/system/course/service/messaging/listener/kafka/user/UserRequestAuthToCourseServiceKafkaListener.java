...
@Slf4j
@Component
public class UserRequestAuthToCourseServiceKafkaListener implements KafkaConsumer<UserRequestAvroModel> {
    private final UserRequestMessageListener userRequestMessageListener;
    private final UserMessagingDataMapper userMessagingDataMapper;

    public UserRequestAuthToCourseServiceKafkaListener(UserRequestMessageListener userRequestMessageListener, UserMessagingDataMapper userMessagingDataMapper) {
        this.userRequestMessageListener = userRequestMessageListener;
        this.userMessagingDataMapper = userMessagingDataMapper;
    }

    @Override
    @KafkaListener(id = "${kafka-consumer-config.auth-service-to-course-service-user-request-group-id}",
            topics = "${course-service.auth-service-user-request-to-course-service-topic-name}")
