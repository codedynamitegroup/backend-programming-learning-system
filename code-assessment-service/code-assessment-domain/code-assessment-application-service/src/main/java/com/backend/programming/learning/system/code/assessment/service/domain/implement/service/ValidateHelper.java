...
public class ValidateHelper {
    ...
    public SharedSolution validateSharedSolution(UUID id){
        Optional<SharedSolution> sharedSolutionOpt =
                sharedSolutionRepository.findById(id);
        if(sharedSolutionOpt.isPresent()){
            return sharedSolutionOpt.get();
        }else{
            log.error("Solution with id: {} not found", id);
            throw new SharedSolutionNotFound("Solution with id: " + id + " not found");
        }
    }
}
