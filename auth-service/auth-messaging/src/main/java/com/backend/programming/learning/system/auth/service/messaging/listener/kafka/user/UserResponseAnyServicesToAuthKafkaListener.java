...
@Slf4j
@Component
public class UserResponseAnyServicesToAuthKafkaListener implements KafkaConsumer<UserResponseAvroModel> {
    private final UserResponseAnyServicesToAuthMessageListener userResponseMessageListener;
    private final UserMessagingDataMapper userMessagingDataMapper;

    public UserResponseAnyServicesToAuthKafkaListener(UserResponseAnyServicesToAuthMessageListener userResponseMessageListener, UserMessagingDataMapper userMessagingDataMapper) {
        this.userResponseMessageListener = userResponseMessageListener;
        this.userMessagingDataMapper = userMessagingDataMapper;
    }

    @Override
    @KafkaListener(id = "${kafka-consumer-config.any-services-user-response-to-auth-service-group-id}",
            topics = "${auth-service.any-services-user-response-to-auth-service-topic-name}")
